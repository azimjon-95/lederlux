...
const ProductList = ({ lang = localStorage.getItem("lang") || 'uz', searchTerm = '' }) => {
    const { data: productsData, isLoading, error: queryError, refetch } = useGetProductsQuery();
    ...
    const handleDeleteProduct = async (productId) => {
        if (!window.confirm(t.deleteConfirm || 'Are you sure you want to delete this product?')) return;
        try {
    ...
    const handleInputChange = (field, lang, value) => {
        if (field === 'price') {
            ...
            if (field === 'title' && lang === 'uz' && formErrors.titleUz && value.trim()) {
    ...
    const saveProduct = async () => {
        ...
        try {
            ...
            formDataToSend.append('applicationAreas[uz]', formData.applicationAreas.uz || '');
            formDataToSend.append('applicationAreas[ru]', formData.applicationAreas.ru || '');
            formDataToSend.append('applicationAreas[en]', formData.applicationAreas.en || '');
            ...
            formDataToSend.append('safetyRequirements[uz]', formData.safetyRequirements.uz || '');
            formDataToSend.append('safetyRequirements[ru]', formData.safetyRequirements.ru || '');
            formDataToSend.append('safetyRequirements[en]', formData.safetyRequirements.en || '');
            formDataToSend.append('storageAndTransport[uz]', formData.storageAndTransport.uz || '');
            formDataToSend.append('storageAndTransport[ru]', formData.storageAndTransport.ru || '');
            formDataToSend.append('storageAndTransport[en]', formData.storageAndTransport.en || '');
            formDataToSend.append('manufacturerWarranty[uz]', formData.manufacturerWarranty.uz || '');
            formDataToSend.append('manufacturerWarranty[ru]', formData.manufacturerWarranty.ru || '');
            formDataToSend.append('manufacturerWarranty[en]', formData.manufacturerWarranty.en || '');
    ...
    const filteredProducts = products.filter(product =>
        Object.values(product.title).some(title => title.toLowerCase().includes(searchTerm.toLowerCase())) ||
        Object.values(product.applicationAreas).some(area => area.toLowerCase().includes(searchTerm.toLowerCase())) ||
        Object.values(product.usageMethod).some(usage => usage.toLowerCase().includes(searchTerm.toLowerCase())) ||
        Object.values(product.safetyRequirements).some(safety => safety.toLowerCase().includes(searchTerm.toLowerCase())) ||
        Object.values(product.storageAndTransport).some(storage => storage.toLowerCase().includes(searchTerm.toLowerCase())) ||
        Object.values(product.manufacturerWarranty).some(warranty => warranty.toLowerCase().includes(searchTerm.toLowerCase()))
    );
        ...
        <div className="pred-table-container">
            ...
            {queryError && (
                <div className="pred-error">
                    {t.error || 'Error'}: {queryError?.data?.message || 'Error loading products'}
                    <button className="pred-btn pred-btn-primary pred-btn-sm" onClick={refetch}>
                        {t.retry || 'Retry'}
                    </button>
                </div>
            )}
                                        ...
                                        <td>
                                            <div className="pred-image-container">
                                                ...
                                                <span className="pred-img-count">
                                                    {t.imagesCount.replace('{count}', product.images.length)}
                                                </span>
                                            </div>
                                        </td>
                                        <td>{product.title[lang]}</td>
                                        <td>${product.price}</td>
                                        <td>{new Date(product.updatedAt).toLocaleDateString(lang === 'ru' ? 'ru-RU' : lang === 'en' ? 'en-US' : 'uz-UZ')}</td>
                                        ...
                                        <td>
                                            <div className="pred-actions">
                                                <button
                                                    ...
                                                    title={t.delete}
                                                >
                                                    {isDeleting ? <Loader2 size={16} className="tfu-spinner" /> : <Trash2 size={16} />}
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
                        ...
                        <div className="pred-modal-body">
                            <div className="pred-form-section">
                                ...
                                <div className="pred-form-group">
                                    ...
                                    <LanguageTabs section="title" activeLanguage={activeTab.title} />
                                    <div className="tab-content">
                                        ...
                                        {formErrors.titleUz && <span className="pred-error">{formErrors.titleUz}</span>}
                                    </div>
                                </div>
                                <div className="pred-form-group">
                                    <label>
                                    ...
                                    <div className="pred-price-input">
                                        ...
                                        />
                                        {formErrors.price && <span className="pred-error">{formErrors.price}</span>}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="pred-form-section">
                                ...
                                <div className="pred-form-group">
                                    ...
                                    <LanguageTabs section="areas" activeLanguage={activeTab.areas} />
                                    <div className="tab-content">
                                        {activeTab.areas === 'uz' && (
                                            <textarea
                                                value={formData.applicationAreas.uz}
                                                onChange={(e) => handleInputChange('applicationAreas', 'uz', e.target.value)}
                                        ...
                                        {activeTab.areas === 'ru' && (
                                            <textarea
                                                ...
                                                onChange={(e) => handleInputChange('applicationAreas', 'ru', e.target.value)}
                                        ...
                                        {activeTab.areas === 'en' && (
                                            <textarea
                                                ...
                                                onChange={(e) => handleInputChange('applicationAreas', 'en', e.target.value)}
                            ...
                            <div className="pred-form-section">
                                ...
                                <div className="pred-form-group">
                                    ...
                                    <LanguageTabs section="usage" activeLanguage={activeTab.usage} />
                                    <div className="tab-content">
                                        {activeTab.usage === 'uz' && (
                                            <textarea
                                                value={formData.usageMethod.uz}
                                                onChange={(e) => handleInputChange('usageMethod', 'uz', e.target.value)}
                                        ...
                                        {activeTab.usage === 'ru' && (
                                            <textarea
                                                ...
                                                onChange={(e) => handleInputChange('usageMethod', 'ru', e.target.value)}
                                        ...
                                        {activeTab.usage === 'en' && (
                                            <textarea
                                                ...
                                                onChange={(e) => handleInputChange('usageMethod', 'en', e.target.value)}
                            ...
                            <div className="pred-form-section">
                                ...
                                <div className="pred-form-group">
                                    ...
                                    <LanguageTabs section="safety" activeLanguage={activeTab.safety} />
                                    <div className="tab-content">
                                        {activeTab.safety === 'uz' && (
                                            <textarea
                                                value={formData.safetyRequirements.uz}
                                                onChange={(e) => handleInputChange('safetyRequirements', 'uz', e.target.value)}
                                        ...
                                        {activeTab.safety === 'ru' && (
                                            <textarea
                                                ...
                                                onChange={(e) => handleInputChange('safetyRequirements', 'ru', e.target.value)}
                                        ...
                                        {activeTab.safety === 'en' && (
                                            <textarea
                                                ...
                                                onChange={(e) => handleInputChange('safetyRequirements', 'en', e.target.value)}
                            ...
                            <div className="pred-form-section">
                                ...
                                <div className="pred-form-group">
                                    ...
                                    <LanguageTabs section="storage" activeLanguage={activeTab.storage} />
                                    <div className="tab-content">
                                        {activeTab.storage === 'uz' && (
                                            <textarea
                                                value={formData.storageAndTransport.uz}
                                                onChange={(e) => handleInputChange('storageAndTransport', 'uz', e.target.value)}
                                        ...
                                        {activeTab.storage === 'ru' && (
                                            <textarea
                                                ...
                                                onChange={(e) => handleInputChange('storageAndTransport', 'ru', e.target.value)}
                                        ...
                                        {activeTab.storage === 'en' && (
                                            <textarea
                                                ...
                                                onChange={(e) => handleInputChange('storageAndTransport', 'en', e.target.value)}
                            ...
                            <div className="pred-form-section">
                                ...
                                <div className="pred-form-group">
                                    ...
                                    <LanguageTabs section="warranty" activeLanguage={activeTab.warranty} />
                                    <div className="tab-content">
                                        {activeTab.warranty === 'uz' && (
                                            <textarea
                                                value={formData.manufacturerWarranty.uz}
                                                onChange={(e) => handleInputChange('manufacturerWarranty', 'uz', e.target.value)}
                                        ...
                                        {activeTab.warranty === 'ru' && (
                                            <textarea
                                                ...
                                                onChange={(e) => handleInputChange('manufacturerWarranty', 'ru', e.target.value)}
                                        ...
                                        {activeTab.warranty === 'en' && (
                                            <textarea
                                                ...
                                                onChange={(e) => handleInputChange('manufacturerWarranty', 'en', e.target.value)}
                ...
                <div className="pred-modal">
                    <div className="pred-modal-content">
                            ...
                            {selectedProduct.images?.length > 0 ? (
                                <div className="pred-gallery-container">
                                    ...
                                    <img
                                        ...
                                        alt={`${selectedProduct.title[lang]} - ${currentImageIndex + 1}`}
                        ...
                        <div className="pred-modal-actions">
                            <button onClick={closeModal} className="pred-btn pred-btn-close">{t.close}</button>
                        </div>
                    </div>
                </div>
            )}
